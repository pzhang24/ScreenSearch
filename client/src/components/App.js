...
class App extends React.Component {
  ...
  handleSearchSubmit = (e) => {
    ...
    const customUrl = `${baseURL}/api/search/multi?query=${this.state.searchTerm}`;
    ...
    Axios.get(customUrl).then(
      (response) => {
        ...
        this.setState((prevState) => {
          return {
            searchResultMsg:
              ...
              ` result${response.data.total_results === 1 ? "" : "s"} for '` +
        ...
        this.setState((prevState) => {
          return { savedSearchTerm: prevState.searchTerm };
        });
  ...
  pageChange = (pageNumber) => {
    const url = `${baseURL}/api/search/multi?query=${this.state.savedSearchTerm}&page=${pageNumber}`;
  ...
  viewInfo = (newID, newType, newName) => {
    ...
    this.setState((state) => ({
      viewItemStack: [
        ...state.viewItemStack,
        { id: newID, type: newType, name: newName },
      ],
    }));
  };
  ...
  closeInfo = () => {
    this.setState((state) => ({
      viewItemStack: [
        ...state.viewItemStack.slice(0, state.viewItemStack.length - 1),
      ],
    }));
  };
  ...
  render() {
    ...
    if (this.state.welcome) {
      const welcomeMessage =
        "Find information on your favourite movies, TV shows, actors and actresses, directors, producers, and more!";
      content = (
        <div className="main">
      ...
      content = (
        <div className="main">
          <ViewInfo
            ...
            id={currentItemId}
            prevName={prevItemName}
            origSearch={this.state.savedSearchTerm}
    ...
    return (
      <div id="App">
        <NavTop
          ...
          disableSearch={this.state.loading}
        />
        {content}
        <Spacer />
        <Footer />
