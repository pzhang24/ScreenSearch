...
const PersonInfo = (props) => {
  ...
  useEffect(() => {
    let mounted = true;
    axios
      .get(`${baseURL}/api/person/${props.personDetails.id}/movie_credits`)
      .then((res) => {
        if (mounted) {
          console.log(res);
          setMovieCastGallery(
            processCreditsGallery(res.data.cast, "person", "cast", "movie")
          );
          setMovieCrewGallery(
            processCreditsGallery(res.data.crew, "person", "crew", "movie")
          );
        }
      })
      .catch((err) => {
    ...
    axios
      .get(`${baseURL}/api/person/${props.personDetails.id}/tv_credits`)
      .then((res) => {
        if (mounted) {
          console.log(res);
          setTvCastGallery(
            processCreditsGallery(res.data.cast, "person", "cast", "tv")
          );
          setTvCrewGallery(
            processCreditsGallery(res.data.crew, "person", "crew", "tv")
          );
        }
      })
      .catch((err) => {
  ...
  return (
    <div>
      ...
      {movieCastGallery.length > 0 && (
        <InfoGallery
          viewInfo={props.viewInfo}
          itemList={movieCastGallery}
          title="Movie Cast Roles"
        />
      )}
      {tvCastGallery.length > 0 && (
        <InfoGallery
          viewInfo={props.viewInfo}
          itemList={tvCastGallery}
          title="TV Cast Roles"
        />
      )}
      {movieCrewGallery.length > 0 && (
        <InfoGallery
          viewInfo={props.viewInfo}
          itemList={movieCrewGallery}
          title="Movie Crew Roles"
        />
      )}
      {tvCrewGallery.length > 0 && (
        <InfoGallery
          viewInfo={props.viewInfo}
          itemList={tvCrewGallery}
          title="TV Crew Roles"
        />
      )}
    </div>
...
function createPersonSummary(details) {
  ...
  const known_for = details.known_for_department
  ...
  if (details.deathday)
    summary.push({ heading: "Deathday", text: `${details.deathday}` });
