...
const MovieInfo = (props) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${baseURL}/api/movie/${props.movieDetails.id}/credits`)
      .then((res) => {
        if (mounted) {
          setCastGallery(
            processCreditsGallery(res.data.cast, "movie", "cast", "person")
          );
          setCrewGallery(
            processCreditsGallery(res.data.crew, "movie", "crew", "person")
          );
        }
      })
      .catch((err) => {
    ...
    axios
      .get(`${baseURL}/api/movie/${props.movieDetails.id}/recommendations`)
      .then((res) => {
        if (mounted) {
          setRecommendedGallery(
            processRecommendedGallery(res.data.results, "movie")
          );
        }
      })
      .catch((err) => {
    ...
    axios
      .get(`${baseURL}/api/movie/${props.movieDetails.id}/similar`)
      .then((res) => {
        if (mounted) {
          console.log(res);
          setSimilarGallery(
